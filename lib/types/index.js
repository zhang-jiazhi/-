...
function readProviderConfigs() {
    ...
    try {
        ...
        const root = parseSimpleYaml(raw);
        const piAi = root['llm-pi-ai'];
    ...
    if (configs.length === 0) {
        configs.push({ provider: 'opencode-go', displayName: 'OpenCode Go 套餐', apiKeyEnv: 'OPENCODE_GO_API_KEY' }, { provider: 'mimo', displayName: '小米 MiMo Token Plan', apiKeyEnv: 'XIAOMI_API_KEY', baseURL: 'https://token-plan-cn.xiaomimimo.com/v1' });
    }
...
async function probeChannel(ctx, config, resolveKey) {
    ...
    if (url.includes('opencode.ai/zen/go') || config.provider === 'opencode-go') {
        ...
        if (key === undefined)
            return { channel: config.provider, kind: 'plan', displayName: config.displayName, error: `未找到 ${config.apiKeyEnv} 凭据` };
    ...
    if (url.includes('api.deepseek.com')) {
        const key = await resolveKey(config.apiKeyEnv);
        if (key === undefined)
            return { channel: config.provider, kind: 'balance', displayName: config.displayName, error: `未找到 ${config.apiKeyEnv} 凭据` };
    ...
    if (url.includes('api.openai.com')) {
        ...
        for (const bucket of buckets) {
            ...
            const body = await probeJson(`https://api.openai.com/v1/usage?start_time=${start}&bucket_width=1d`, { authorization: `Bearer ${key}` });
    ...
    if (url.includes('api.anthropic.com')) {
        ...
        for (const bucket of buckets) {
            ...
            const body = await probeJson(`https://api.anthropic.com/v1/organizations/usage/costs?start_time=${start}&bucket_width=1h`, { 'x-api-key': key, 'anthropic-version': '2023-06-01' });
            const rows = body['data'] ?? [];
            let input = 0;
            let output = 0;
            for (const row of rows) {
...
export function computeSummary(records) {
    ...
    for (const record of records) {
        ...
        if (channelEntry !== undefined) {
            ...
            if (!channelEntry.models.includes(record.model))
                channelEntry.models.push(record.model);
        }
        else {
    ...
    void (async () => {
        try {
            ...
            for (const entry of sessions) {
                if (entry.live)
                    continue; // live sessions are covered by the listener
                try {
